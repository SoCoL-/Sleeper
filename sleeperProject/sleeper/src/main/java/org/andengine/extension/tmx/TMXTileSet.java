...
public class TMXTileSet implements TMXConstants {
	...
	public ITextureRegion getTextureRegionFromGlobalTileID(final int pGlobalTileID) {
		...
		final float texturePositionX = this.mMargin + (this.mSpacing + this.mTileWidth) * tileColumn;
		final float texturePositionY = this.mMargin + (this.mSpacing + this.mTileHeight) * tileRow;

		return new TextureRegion(this.mTexture, texturePositionX, texturePositionY, this.mTileWidth, this.mTileHeight);
	}

	private static int determineCount(final int pTotalExtent, final float pTileExtent, final int pMargin, final int pSpacing) {
		int count = 0;
